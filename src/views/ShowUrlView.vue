...
watch(
	...
	async () => {
		try {
			...
			devices.value = [...response.data.top_devices, ...devices.value].slice(0, 3).reverse();
			...
			datasets: [
				{
					...
					backgroundColor: browsers.value.map((el) => getColorBrowser(el.browser)),
					hoverOffset: 0,
					spacing: 4,
				},
			],
